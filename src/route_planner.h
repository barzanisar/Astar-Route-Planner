...
class RoutePlanner {
 public:
  ...
  /**
   * @brief Finds shortest path from start_node to end_node via A star search
   * method.
   */
  void AStarSearch();
   ...
   * individually. They are not intended to be used by the main.cpp
   * ***********************************************************************************/

  /**
   * @brief Expands the current node by adding all its unvisited neighbors to
   * the open list.
   *
  ...
  /**
   * @brief Calculates h value i.e. the distance from the input node to the
   * end_node.
   *
   ...
   * constructs the final path by following the chain of parents of nodes until
   * the starting node is found. It also calculates the distance of the final
   * path.
   *
   * @return final path
   */
